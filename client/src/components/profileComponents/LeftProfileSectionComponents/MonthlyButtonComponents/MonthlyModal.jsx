...
const MonthlyModal = ({
  ...
  const { t: MonthModalTranslate } = useTranslation('MonthlyModal')

  // Memoize scrollbar style creation function
  const styleScrollbar = useCallback(() => {
    const style = document.createElement('style')
    style.innerHTML = `
      .monthlyModalBody::-webkit-scrollbar {
        width: 8px;
      }
      .monthlyModalBody::-webkit-scrollbar-thumb {
        background-color: #333;
        border-radius: 4px;
      }
      .monthlyModalBody::-webkit-scrollbar-thumb:hover {
        background-color: #555;
      }
      .monthlyModalBody::-webkit-scrollbar-track {
  ...
  useEffect(() => {
    const handleWheel = event => {
      const modalBody = document.querySelector('.monthlyModalBody')
      if (modalBody) {
    ...
    >
      ...
      >
        <ModalHeader textTransform={'uppercase'}>
          <Heading
            ...
            title={
              month
                ? `${MONTHS[month - 1]} ${year}`
                : MonthModalTranslate('monthlyStats')
            }
            tag={MonthModalTranslate('history')}
            marginBottom={'0'}
          />
        </ModalHeader>
        ...
        <ModalBody
          ...
          pl={'7px'}
          pr={'0'}
          className="monthlyModalBody"
        >
          {notInTheMonth ? (
            <Heading
              title={
                loginedUserProfile
                  ? MonthModalTranslate('noDataCurrentUser')
                  : MonthModalTranslate('noDataOtherUser')
              }
              color="white"
            />
          ) : (
...
export default React.memo(MonthlyModal)
