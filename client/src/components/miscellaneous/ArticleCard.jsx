...
const ArticleCard = ({
  ...
  const CardContent = () => (
    <Flex flexDirection={'column'} mr={isMobileListView ? 8 : 0}>
      <Heading
        fontSize={isMobileListView ? '14px' : '18px'}
        mb="10px"
        color="#ffffff"
      >
  ...
  if (viewMode === 'list' && !isMobileListView) {
    return (
      <Box
        as="article"
        display="flex"
  ...
  if (viewMode === 'list' && isMobileListView === true) {
    return (
      <Box
        as="article"
        display="flex"
      ...
      >
        ...
        {!isLoading && onRemove && (
          <IconButton
            ...
            right="5px"
            size="sm"
            colorScheme={isMobileListView ? 'transparent' : 'red'}
            onClick={e => {
              e.stopPropagation()
              onRemove(article._id)
            }}
